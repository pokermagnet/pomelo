...
Channel.prototype.leave = function(uid, sid) {
  if(uid && !sid){
    ...
    var group, i, l;
    for(var serverId in groups) {
      group = groups[serverId];
      if(group.indexOf(uid)>=0){
        sid = serverId;
        break;
      }
    }
...
/**
 * Push different messages to some of the members in the channel
 *
 * @param {String} route message route
 * @param {Object} msgs  messages respect to sessionId-s
 * @param {Object} opts  user-defined push options, optional
 * @param {Function} cb  callback function
 */
 Channel.prototype.pushPrivateMessages = function(route, msgs, opts, cb) {
  if(this.state !== ST_INITED) {
    utils.invokeCallback(new Error('channel is not running now'));
    return;
  }
  ...
  for(var memberUid in msgs){
    var group = generateGroup(memberUid, this.groups);
    if(group){
      // take care for cb; better is -> cb = undefined
      sendMessageByGroup(this.__channelService__, route, msgs[memberUid], group, opts, cb);
    }
  }

  function generateGroup(uid, groups) {
    for(var sid in groups) {
    ...
    loadAllFromStore(self, genKey(self), function(err, list) {
      if(!!err) {
       ...
       for(var i=0; i<list.length; i++) {
        ...
        load(list[i], name);
        if(self.store.updateKeys instanceof Function) {
          self.store.updateKeys(self.app, self.channels[name])
        };
      }
      utils.invokeCallback(cb);
    }
